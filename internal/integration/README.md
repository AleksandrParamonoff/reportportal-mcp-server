...
1. Create a new JSON file in `testdata/` (or `testdata/collections/` for Postman suites).
...
You can also define tests using Postman Collection format. Place collection files in `testdata/collections/` directory. The test runner will automatically discover and run them.
